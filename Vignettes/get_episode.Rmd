...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "figures/get_episode-1",
  out.width = "100%"
)
```


In the context of this package, the `get_episode()` function returns a data.frame in R implementing the episode approach to measure growth and decline episodes of a V-Dem index variable as suggested in @lott_academic_2023. By doing so, this `get_episode()` function differs from the approach implemented in the "Episode of Regime Transformation" approach as published in @maerz_episodes_2023 and available at @institute_vdeminstituteert_2023. 

## The `get_episode()` function

By default, `get_episode()` returns a data.frame in R implementing the episode approach to measure growth and decline episodes of the Academic Freedom Index as suggested in @lott_academic_2023: 
...
The `get_episode` function considers measurement uncertainty in the measurement of the index variables. By doing so,  `get_episode` function controls for overlapping uncertainty intervals (before the start of an episode and at the end of an episode). However, when not considering measurement uncertainty in the measurement of episodes, the cutoff point needs to be more demanding to reduce the risks of measurement error. User then should use the `get_episode_wo_CI` function from this package. 
...
The `data` argument is a way to use another V-Dem dataset than the dataset coming with the package (latest V-Dem version). Users can also use `get_episode()` function with other datasets. However, the function was created for working with V-Dem data. 
...
The `year_turn` argument enables users to change the parameter that is necessary to trigger the end of an episode. By default, this value is set to 0.03. The `get_episode()` function terminates an episode when there is a temporary stagnation on the AFI with no further increase/decline of `start_incl` points in four years or when the AFI decreases/increases by `year_turn` points, from one year to the next. 
