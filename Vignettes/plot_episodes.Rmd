...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/plot_episode-",
  out.width = "100%"
)
```
...
By using the default values, users get a ggplot() object comparable to Figure 3 and 5 presented in @lott_academic_2023. However, users are also able to customize the default parameters and use other V-Dem index variables than the Academic Freedom Index. 
...
Only entries from the country_name column in the V-Dem data set are accepted. The default country is "Sweden". 
...
By default, the label is "Academic Freedom Index". The label should correspond to the `variable` chosen.


```{r}
fig4 <- plot_episodes(varibale = "v2x_libdem", var_label = "Liberal Democracy Index", country = "India")
fig4
```
