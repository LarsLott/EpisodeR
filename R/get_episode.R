...
get_episode <- function(data = EpisodeR::vdem,
                    start_incl = 0.01,
                    cum_incl = 0.1,
                    year_turn = 0.03,
                    variable = "v2xca_academ")
{
  ...
  full.df <- as.data.frame(full.df)
    ...
    # second step: create dummy in which decline was greater than Z
      ...
      group_by(country_name) %>%
      mutate(auto_confidence = lag(last_VAR_codelow) - last_VAR_codehigh) %>%
      ungroup()%>%
      mutate(auto_dum = last_VAR - lag(last_VAR)) %>%
      dplyr::select(-c(group_id)) %>%
      group_by(episode_id) %>%
      mutate(decline_episode = ifelse(start_auto==1 & first(auto_dum) <= -cum_incl & first(auto_confidence) >0 , 1, 0 )) %>%
      ...
      dplyr::mutate(variable_pre_ep_year = ifelse(decline_episode==1, ifelse(year == dplyr::first(year), 1, 0), 0),
    ...
    full.df <- full.df %>%
      ...
      mutate(last_VAR = last(get(variable)),
             ...
             last_VAR_codehigh = last(get(paste0(variable, "_codehigh"))))%>%
      group_by(country_name) %>%
      mutate(auto_confidence = lag(last_VAR_codehigh) - last_VAR_codelow) %>%
      ungroup()%>%
      mutate(auto_dum = last_VAR - lag(last_VAR)) %>%
      dplyr::select(-c(group_id)) %>%
      group_by(episode_id) %>%
      mutate(increase_episode = ifelse(start_auto==1 & first(auto_dum) >= cum_incl & first(auto_confidence) < 0 , 1, 0 )) %>%
